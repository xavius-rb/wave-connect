...
RSpec.describe VersionControl::Bridge do
  ...
  describe '#commits' do
    ...
    it 'calls client.commits with the provided owner and repo' do
      expect(mocked_client).to receive(:commits).with(owner: owner, repo: repo,
                                                      per_page: 2).and_return(successful_response)
  ...
  describe '#repository_content' do
    it 'calls client.repository_content with the provided owner, repo, and path' do
      ...
      repo = 'example_repo'
      path = 'example_path'
      expect(mocked_client).to receive(:repository_content).with(owner: owner, repo: repo,
                                                                 path: path).and_return(successful_response)
  ...
  describe '#pull_requests' do
    it 'calls client.pull_requests with the provided owner and repo' do
      owner = 'example_owner'
      repo = 'example_repo'
      expect(mocked_client).to receive(:pull_requests).with(owner: owner, repo: repo,
                                                            per_page: 2).and_return(successful_response)
